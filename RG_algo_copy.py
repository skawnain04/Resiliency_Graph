...
def remove_effect_from_action(parsed_actions, action_name, effect):
    def effect_matches(eff, effect_to_remove):
        """
        Given an effect tuple, e.g. ('and', ('fault-at_r1_l2',), ('exe-fault-at_r1_l2',)),
        return True if any inner token equals effect_to_remove or "exe-" + effect_to_remove.
        """
        # We expect eff[0] to be the "and" keyword.
        for elem in eff[1:]:
            # If elem is a tuple, check each token inside.
            if isinstance(elem, tuple):
                for token in elem:
                    if token == effect_to_remove or token == "exe-" + effect_to_remove:
                        return True
            elif isinstance(elem, str):
                if elem == effect_to_remove or elem == "exe-" + effect_to_remove:
                    return True
        return False
...
def current_state_sim(parser, action, sim_state):
    postCond_act = find_postCond(parser, action, type="base")
...
domain_estimator = network+"-domain_all_faults.pddl"
problem_estimator = network+"-problem.pddl"
...
domain_sim = network+"-domain.pddl"
problem_sim = network+"-problem.pddl"
start_time = datetime.now()
...
while 1:
    ...
    while(len(estimator_actions)>0):
        ...
        #Pop actions from Optimistic Model to check the next state of the Simulator
        action = estimator_actions.pop(0)
        print("--POP Action from Estimated plan--")
        print(action)

        preCond_act_est = find_preCond(parser_est, action, type="opt")
        ...
        if is_subset_of_sim:
            ...
            if (sim_state!=remove_var_est_state):
                predicates_remove_from_est = remove_var_est_state - sim_state
